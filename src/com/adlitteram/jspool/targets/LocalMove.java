...
public class LocalMove extends AbstractTarget {
    ...
    private static final Logger logger = LoggerFactory.getLogger(LocalMove.class);
    //
...
//    private JTextField target3Field;
//    private JTextField target4Field;
    ...
    public void init(Channel ch) {
        ...
        if (dirs.length() > 0 && !dirs.endsWith(";")) {
            dirs += ";";
        }
    ...
    public int run(String srcDir, SourceFile srcfile) {
        ...
        if (rename) {
            if (pattern == null) {
                try {
                    ...
                    channel.logInfo(channel.getStringProp(Channel.ID) + " - " + Message.get("localmove.message7"));
                }
            }
            if (pattern != null) {
        ...
        if (dirs != null && dirs.length() > 0) {
            for (String dir : dirs.split(";")) {
                if (!channel.isStopped() && dir.length() > 0) {
    ...
    private boolean doCopy(String srcDir, SourceFile srcfile, File trgDir, String trgname, String tmpExt) {

        // Test si le repertoire source et cible sont identiques
//		File sDir = new File (srcDir) ;
//		if ( sDir.equals(trgDir))  {
//			XLog.info(Message.get("localmove.message8")) ;
//			return false ;
//		}
        ...
        if (trgSubDir) {
            ...
            trgDir = new File(FilenameUtils.getFullPath(trgDir.getPath() + "/" + filePath));
        }

        // Creation du repertoire destination si necessaire
        if (!trgDir.exists()) {
            if (trgDir.getPath().length() == 0) {
                channel.logInfo(channel.getStringProp(Channel.ID) + " - " + Message.get("localmove.message1"));
                return false;
            }
            channel.logInfo(channel.getStringProp(Channel.ID) + " - " + Message.get("localmove.message2", new String[]{trgDir.getPath()}));
        ...
        if (srcfile.exists() && srcfile.canRead()) {
            try {
                ...
                channel.logInfo(channel.getStringProp(Channel.ID) + " - " + Message.get("localmove.message3", args3));
                return true;
            }
        ...
        if (!channel.isStopped()) {
            channel.logInfo(channel.getStringProp(Channel.ID) + " - " + Message.get("localmove.message4", args3));
        }
...
//        target1Field = new JTextField(channel.getStringProp(TARGET_DIR1), 30);
//        JButton browse1Button = Utils.createDirButton(Main.getMainFrame(), target1Field);
//        target2Field = new JTextField(channel.getStringProp(TARGET_DIR2), 30);
//        JButton browse2Button = Utils.createDirButton(Main.getMainFrame(), target2Field);
//        target3Field = new JTextField(channel.getStringProp(TARGET_DIR3), 30);
//        JButton browse3Button = Utils.createDirButton(Main.getMainFrame(), target3Field);
        ...
        renameCheck.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                setEnabledRename(renameCheck.isSelected());
            }
        });
    ...
    public boolean setParameters() {
        channel.setProperty(TARGET_DIRS, targetArea.getText().replace('\n', ';').replaceAll(";+", ";").replaceFirst("^;", ""));
//        channel.setProperty(TARGET_DIR1, target1Field.getText());
//        channel.setProperty(TARGET_DIR2, target2Field.getText());
//        channel.setProperty(TARGET_DIR3, target3Field.getText());
    ...
    public String rename(Pattern pattern, String src, String out) {
        ...
        if (m.matches()) {
            try {
                ...
                channel.logInfo(channel.getStringProp(Channel.ID) + " - " + Message.get("localmove.message6", new String[]{src}));
                return src;
            }
        }
        else {
            channel.logInfo(channel.getStringProp(Channel.ID) + " - " + Message.get("localmove.message5"));
            return src;
        }
    }
}
