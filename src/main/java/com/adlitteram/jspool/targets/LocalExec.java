...
public class LocalExec extends AbstractTarget {
  ...
  public int run(String srcDir, SourceFile srcfile) {
    ...
    String str =
        channel.getStringProp(EXECTARGET_NAME)
            + " "
  ...
  public JPanel buildPanel(Dialog parent) {
    ...
    removeCheck =
        new JCheckBox(
            Message.get("localexec.message5"), channel.getBooleanProp(EXECTARGET_REMOVE, true));
    ...
    syncCheck =
        new JCheckBox(
            Message.get("localexec.message4"), channel.getBooleanProp(EXECTARGET_SYNC, true));
