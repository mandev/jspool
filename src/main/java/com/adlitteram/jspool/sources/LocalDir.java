...
public class LocalDir extends AbstractSource {
  ...
  protected static final String[] SORT_ARRAY = {
  ...
  public boolean doRun(String srcDir, LocalFile srcFile, AbstractTarget target) {
    if (!srcFile.exists()) {
      channel.logInfo(
          channel.getStringProp(Channel.ID)
              + " - "
              + Message.get("channel.run1", new String[] {srcFile.getPath()}));
      return false;
    }
    ...
    if (!srcFile.isDirectory()) {
      channel.logInfo(
          channel.getStringProp(Channel.ID)
              + " - "
              + Message.get("channel.run2", new String[] {srcFile.getPath()}));
      return false;
    }
  ...
  public JPanel buildPanel() {
    ...
    checksumCheck =
        new JCheckBox(Message.get("source.checksum"), channel.getBooleanProp(SOURCE_CHECK, true));
    checksumCheck.setBorder(BorderFactory.createEmptyBorder(3, 0, 3, 0));

    sortCombo = new JComboBox<>(SORT_ARRAY);
