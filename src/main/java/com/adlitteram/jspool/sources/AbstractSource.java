...
public abstract class AbstractSource {
  ...
  public abstract boolean run(AbstractTarget target);
  ...
  public boolean processDir(String srcDir, SourceFile dir, AbstractTarget target) {
    ...
    if (srcFiles == null) {
      ...
      channel.logInfo(
          channel.getStringProp(Channel.ID) + " - " + Message.get("channel.process4", args));
      return false;
    }
    ...
    for (int i = 0; i < maxFiles; i++) {
      ...
      if (srcFiles[i].isDirectory()) {
        if (channel.listSubDir()) {
          ...
          if (delay > 0
              && (System.currentTimeMillis() - srcFiles[i].lastModified()) > (delay * 1000)
              && srcFiles[i].listFiles(null).length == 0) {
            String[] args = {srcFiles[i].getPath()};
            channel.logInfo(
                channel.getStringProp(Channel.ID) + " - " + Message.get("channel.process7", args));
            srcFiles[i].delete();
          } else {
      ...
      if (!channel.zeroLength() && length == 0) {
        channel.logInfo(
            channel.getStringProp(Channel.ID) + " - " + Message.get("channel.process1", args));
        continue;
      }
      ...
      if (srcFile == null) {
        channel.logInfo(
            channel.getStringProp(Channel.ID) + " - " + Message.get("channel.process3", args));
        ...
        if (srcFile.isStabilized()) {
            ...
            case AbstractTarget.FAIL:
              channel.logInfo(
                  channel.getStringProp(Channel.ID)
                      + " - "
                      + Message.get("channel.process5", args));
              status = false;
          ...
          channel.logInfo(
              channel.getStringProp(Channel.ID) + " - " + Message.get("channel.process6", args));
        }
      }
    }
    return status;
    ...
    for (int i = 0; i < maxFiles; i++) {
      ...
      if (files[i].isDirectory()) {
        if (channel.listSubDir()) {
          long delay = channel.deleteSubDirDelay();
          if (delay > 0
              && (System.currentTimeMillis() - files[i].lastModified()) > (delay * 1000)
              && files[i].listFiles(null).length == 0) {
      ...
      if (scrcFile.isStabilized()) {
        switch (target.run(".", scrcFile)) {
          ...
          case AbstractTarget.FAIL:
            channel.logInfo(
                channel.getStringProp(Channel.ID) + " - " + Message.get("channel.process5", args));
            status = false;
