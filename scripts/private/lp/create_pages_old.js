...
function convertToJpeg(srcFile, dstFile)  {
    ...
    var exe = "C:/Program Files/gs/gs9.04/bin/gswin32c.exe " ;
...
function cropPlateToPages(file)  {
	...
	var w = Math.round(d.width * (260+20) / 644) ;
	var h = Math.round(d.height * (360+20) / 410) ;
	var x0 = Math.round(d.width * 46 / 644) ;
...
function splitPdfPlate(file) {
    ...
    pdfTool.addFilter(new CropMargin(leftPdfFile.getPath(), 45* NumUtils.MMtoPT, 20* NumUtils.MMtoPT, 10* NumUtils.MMtoPT, 317* NumUtils.MMtoPT ));
    pdfTool.execute(file);
    convertToPdf(leftPdfFile)  

    pdfTool = new PdfTool();
    pdfTool.addFilter(new CropMargin(rightPdfFile.getPath(), 326* NumUtils.MMtoPT, 20* NumUtils.MMtoPT, 10* NumUtils.MMtoPT, 37* NumUtils.MMtoPT ));
    pdfTool.execute(file);
...
// Process the XML file that contains the pagination structure
function processXml(file) {
	_print("processXml starting");
	...
	var dstFile = new File(PAGIN_DIR, parutionDate + "_" + PAGIN_NAME) ;
	_print("copie : " + _srcFile.getName() + " vers " + dstFile.getPath()) ;
	FileUtils.copyFile(file, dstFile) ;
	
	dstFile = new File(SUIVI_DIR, parutionDate + "_" + PAGIN_NAME) ;
	_print("copie : " + _srcFile.getName() + " vers " + dstFile.getPath()) ;
	FileUtils.copyFile(file, dstFile) ;
	
	dstFile = new File(PORTAIL_DIR, parutionDate + "_" + PAGIN_NAME) ;
	_print("copie : " + _srcFile.getName() + " vers " + dstFile.getPath()) ;
	FileUtils.copyFile(file, dstFile) ;
	
	_print("processXml done");
}
