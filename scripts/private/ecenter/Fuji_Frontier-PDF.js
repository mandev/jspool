...
var frontiers = {
    ...
    "PRESS-6B": {device: "LP-2500PsRGB", path: "//FRONTENDDELL/C8Print/"},
...
function rasterPdf() {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        _print("dstFile: " + dstFile);

        var opt = ["-q", "-dNOPROMPT", "-dBATCH", "-dNOPAUSE", "-dUseCIEColor", "-sDEVICE=jpeg", "-dUseCropBox", "-dJPEGQ=95",
            "-dTextAlphaBits=4", "-dGraphicsAlphaBits=4", "-r300", "-o", dstFile.getName(), srcFile.getPath()];
...
function getSizeName(width, height) {
    var fmt =  Math.round(width * P2MM) + "X" + Math.round(height * P2MM);
    if ( fmt == "152x102" ) return "15X10" ;
    if ( fmt == "152x117" ) return "15n11" ;
    if ( fmt == "88x104" ) return "10X9" ;
    return fmt ;
}
...
function splitPdf() {
    var pdfFile = new File(INPUT_DIR + "/" + tirage, product + ".pdf");
    if (pdfFile.exists() && pdfFile.isFile() && pdfFile.canRead()) {
        FileUtils.forceMkdir(getTempDir());
        var pdfTool = new PdfTool();
        var splitPageFilter = new SplitPage(getTempDir() + "/{BASE}_{COUNT}.pdf", 1, "%03d");
...
function parseCSV(csvFile) {
    ...
    while (iterator.hasNext()) {
        ...
        try {
            ...
            _print("frontier: " + frontier + " - product: " + product + " - pdf: " + product + ".pdf - tirage: " + tirage);
