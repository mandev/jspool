...
function createXMLvirgule() {
    ...
    var article = new Element("article") ;
    var tables = new Element("texte") ;
	var hspc = String.fromCharCode(8202);
    for (var i in courseArray) {
        var course = courseArray[i] ;
        ...
        if (course.number != "") {
          ...
          //  td01.addAttribute(new Attribute("colspan","2"));
 	      //  td01.appendChild(course.number);
          var sp = new Element("span") ;
	    sp.addAttribute(new Attribute("class", "coursenumber" ));
		sp.appendChild(course.number);
		var sup = new Element("sup");
	    sup.appendChild((course.number.equals("1")) ? "re" : "e"); 
	    sp.appendChild(sup);
	    sp.appendChild(" course ") ;
		//sp.appendChild(hspc) ;
            table0.appendChild(sp) ;

            // Display starters
            var b1 = new Element("b") ;
            // b1.addAttribute(new Attribute("class", course.classe + "_tab_starters" )) ;

            // var td10 = new Element("td") ;
            // td10.addAttribute(new Attribute("class", course.classe + "_horseName" )) ;
            // var b10 = new Element("b") ;
            b1.appendChild(course.starters[0].place + ". " + course.starters[0].name + " (" + course.starters[0].number + "), ") ;
            table0.appendChild(b1) ;
			
			// td10.appendChild(b10) ;
            table0.appendChild(course.starters[0].driver + ", ") ;
            
            table0.appendChild("G. " + course.gains.gag1) ;
            
			table0.appendChild(" P. " + course.gains.pl11 + hspc + "; ") ;
            // table0.appendChild(tr2) ;

            var b2 = new Element("b") ;
			
			b2.appendChild(course.starters[1].place + ". " + course.starters[1].name + " (" + course.starters[1].number + "), "); 
			table0.appendChild(b2);
			table0.appendChild(course.starters[1].driver + ", ") ;
            
			if (course.starters[0].place == course.starters[1].place) table0.appendChild("G. " + course.gains.gag2) ;
            else table0.appendChild("P. " + course.gains.pla2 + hspc + "; ") ;
            ...
            var b3 = new Element("b") ;
            b3.appendChild(course.starters[2].place + ". " + course.starters[2].name + " (" + course.starters[2].number + "), ");
			table0.appendChild(b3);
			table0.appendChild(course.starters[2].driver + ", ") ;
            table0.appendChild("P. " + course.gains.pla3 + hspc + "; ") ;
            ...
            if (course.starters.length > 3) {
                var b4 = new Element("b") ;
                // tr5.addAttribute(new Attribute("class", course.classe + "_tab_starters" )) ;
                // var td50 = new Element("td") ;
                // td50.addAttribute(new Attribute("class", course.classe + "_tab_starters" )) ;
                b4.appendChild(course.starters[3].place + ". " + course.starters[3].name + " (" + course.starters[3].number + "), ");
				table0.appendChild(b4);
				table0.appendChild(course.starters[3].driver + ". ") ;
				// tr5.appendChild(td50) ;
                // table0.appendChild(tr5) ;
				...
				if ((testtypecourse.indexOf("Quinté+")>=0)||(course.gains.pknu !="0")) {
				//		_print("OK traitement particulier pour " + testtypecourse);
						var b5 = new Element("b") ;
						// b5.addAttribute(new Attribute("class", course.classe + "_tab_starters" )) ;
						// var td50 = new Element("td") ;
						// td50.addAttribute(new Attribute("class", course.classe + "_tab_starters" )) ;
						b5.appendChild(course.starters[4].place + ". " + course.starters[4].name + " (" + course.starters[4].number + "), ") ;
						table0.appendChild(b5);
						table0.appendChild(course.starters[4].driver + ". ") ;
						}
				// fin de traitement cas quinte et pick5 - SH - le 09072012
            }

            if (course.starters.length < 8) {
                table0.appendChild("Coup. ord. " + course.gains.cpor +
                    ". Trio ord. " + course.gains.tror + ". " + NP + " " + course.starters[0].time + ".") ;
            }
            else if (course.starters[1].place == course.starters[2].place) {
                table0.appendChild("Coup. gag. (" + course.starters[0].number + "-" + course.starters[1].number + ") " + course.gains.cg12 +
                    " (" + course.starters[0].number + "-" + course.starters[2].number + ") " + course.gains.cg13 +
                    ". Trio (" + course.starters[0].number + "-" + course.starters[1].number + "-" + course.starters[2].number + ") : " +
                    course.gains.trio + ". " + NP + " " + course.starters[0].time + ".") ;
            }
            else if (course.starters[2].place == course.starters[3].place){
                table0.appendChild("Coup. gag. " + course.gains.cg12 + ". Coup. pl. (" +
                    course.starters[0].number + "-" + course.starters[1].number + ")" + hspc + ": " + course.gains.cp12 +
                    " (" + course.starters[0].number + "-" + course.starters[2].number + ") " + course.gains.cp13 +
                    " (" + course.starters[0].number + "-" + course.starters[3].number + ") " + course.gains.cp14 +
                    " (" + course.starters[1].number + "-" + course.starters[2].number + ") " + course.gains.cp23 +
                    " (" + course.starters[1].number + "-" + course.starters[3].number + ") " + course.gains.cp24 +
                    ". Trio (" + course.starters[0].number + "-" + course.starters[1].number + "-" + course.starters[2].number + ")" + hspc + ": " + course.gains.trio + " (" + course.starters[0].number + "-" + course.starters[1].number + "-" + course.starters[3].number + ")" + hspc + ": " + course.gains.trdh + ". " + NP + " " + course.starters[0].time + ".") ;
            }
            else {
                table0.appendChild("Coup. gag. " + course.gains.cg12 + ". Coup. pl. (" +
                    course.starters[0].number + "-" + course.starters[1].number + ")" + hspc + ": " + course.gains.cp12 +
                    " (" + course.starters[0].number + "-" + course.starters[2].number + ") " + course.gains.cp13 +
                    " (" + course.starters[1].number + "-" + course.starters[2].number + ") " + course.gains.cp23 +
                    ". Trio (" + course.starters[0].number + "-" + course.starters[1].number + "-" + course.starters[2].number + ")" + hspc + ": " +          course.gains.trio + ". " + NP + " " + course.starters[0].time + ".") ;
            }
            if (course.gains.pknu != "0") { table0.appendChild(" PICK 5 (" + course.gains.pknu + ")" + hspc + ": " + course.gains.pkva + "."); }
            //tr6.appendChild(td60) ;
            //table0.appendChild(tr6) ;
...
//    printXML() ;
    ...
    var document2 = createXMLvirgule() ;	
    ...
    for (i in OUTPUT_DIR) {
		...
		var file2 = new File(OUTPUT_DIR[i], (reunion.date + "_" + _srcFile.getName()).replace(".ok","BIS.xml")) ;
    	writeXML(document, file) ;
		writeXML(document2, file2) ;
    }
//    }
//    catch(e) {
//        _print(e) ;
