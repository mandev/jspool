...
function cleanStoryFile(file) {
    ...
    content = content.replaceAll("\\<\\?EM-dummyText.*?\\?\\>", "");
...
function buildZip() {
    ...
    if (dir.exists()) {
    	   var zipOutDir = ZIPOUT_DIR.split(";");
        for (var i in zipOutDir) {
           var zipFile = new File(zipOutDir[i] + OUTPUT_NAME + ".zip");
...
function processZip() {
    ...
    var files = new File(ZIP_DIR, FilenameUtils.getBaseName(ZIP_DIR)).listFiles();
    for (var i in files) {
    	   if (FilenameUtils.isExtension(files[i].getName(), "xml")) {
		  ...
		  try {
	           ...
	           _print("processZip - loid: " + loid);
	       	 OUTPUT_NAME = loid;
	           var type = itemNode.getAttributeValue("eomType");
...
function checkHas2Send(node) {
    var section = getValue(node, "metadata/PubData/Paper/Section");
    ...
    if ((section != "Val-de-Marne")
            ...
            && (section != "Essonne")
            && (section != "Oise")) {
        var nodes = node.query("systemMetadata/tl/t");
...
function processStory(storyFile, itemNode, linkedItems) {
    ...
    // photo-group 
    var nodes = itemNode.query("content/photo-groupe");
    ...
    // photo inside encadre
    var photoEncadreNodes = itemNode.query("content/texte/pbox [@class='encadre']");
    for (var j = 0; j < photoEncadreNodes.size(); j++) {
        processInlinePhoto(photoEncadreNodes.get(j), linkedItems, "imageinl", 1);
    }

    // photo-alt1,2,3,4,5 
    for (var i = 1; i <= 5; i++) {
        var photoNodes = itemNode.query("content/photo-alt" + i);
    ...
    // photo-inline 
    var photoInlineNodes = itemNode.query("content/texte/pbox [@class='photo']");
    for (var j = 0; j < photoInlineNodes.size(); j++) {
        processInlinePhoto(photoInlineNodes.get(j), linkedItems, "imageinl", 1);
    }

    // graphic-inline 
    var graphicInlineNodes = itemNode.query("content/texte/pbox [@class='infographie']");
...
function writeStory(storyNode, linkedItems) {
    _print("writeStory : titre = " + getValue(storyNode, "content/titraille/titre"));

    var charsCountElt = createElement("charsCount", getValue(storyNode, "systemMetadata/props/charsCount"));
    var wordCountElt = createElement("wordCount", getValue(storyNode, "systemMetadata/props/wordCount"));
    ...
    metaElement.appendChild(createElement("issueDate", getValue(storyNode, "metadata/PubData/Paper/date_Publication")));
    ...
    var city = getValue(storyNode, "metadata/General/GeographicalPlaces/City_Name");
    if ((city != null) && (city != "")) {
        if (city.indexOf("/") > 0) {
            var tmp = city.split("/");
            ...
            metaElement.appendChild(createElement("zipcode", tmp[1]));
        	  ...
        	  var tmp0 = city.substr(0, city.lastIndexOf("("));
            var tmp1 = city.substr(city.lastIndexOf("(")+1);
            metaElement.appendChild(createElement("city", tmp0.trim()));
            metaElement.appendChild(createElement("zipcode", (tmp1) ? tmp1.replace(')', '') : ""));
        }
    } else {
        metaElement.appendChild(createElement("city", ""));
        metaElement.appendChild(createElement("zipcode", ""));
    }
    metaElement.appendChild(createElement("province", getValue(storyNode, "metadata/General/GeographicalPlaces/Province_Name")));
    metaElement.appendChild(createElement("country", getValue(storyNode, "metadata/General/GeographicalPlaces/Country")));
    var address = getValue(storyNode, "metadata/General/GeographicalPlaces/Address");
    metaElement.appendChild(createElement("address", address.replace('_', ' ').replace('_', ' ')));

    var catElement = new Element("categories");
    catElement.appendChild(createElement("category", getValue(storyNode, "metadata/PubData/Paper/Section") + ""));
    metaElement.appendChild(catElement);

    var keyElement = new Element("keywords");
    keyElement.appendChild(createElement("keyword", getValue(storyNode, "metadata/General/DocKeyword") + ""));
    metaElement.appendChild(keyElement);

    var authorsElement = new Element("authors");
    var txt = trim(getValue(storyNode, "metadata/General/Custom_by-line")) + "";
    ...
    webElement.appendChild(createElement("paying", getValue(storyNode, "metadata/PubData/Web/Paying") + ""));
    webElement.appendChild(createElement("theme", getValue(storyNode, "metadata/PubData/Web/Theme") + ""));
    webElement.appendChild(createElement("status", getValue(storyNode, "metadata/PubData/Web/Status") + ""));
    webElement.appendChild(createElement("comment", getValue(storyNode, "metadata/PubData/Web/Comment") + ""));
    webElement.appendChild(createElement("profile", getValue(storyNode, "metadata/PubData/Web/Profile") + ""));
//    webElement.appendChild(createElement("priority", getValue(storyNode, "metadata/PubData/Digital/Priority") + ""));
//    if ((city == null) || (city == "")) {
//        webElement.appendChild(createElement("primeRubric", ""));
//    }
    webElement.appendChild(createElement("primeRubric", getValue(storyNode, "metadata/PubData/Digital/Priority") + ""));
    
    var topicsList = getValue(storyNode, "metadata/PubData/Web/TopicsList") + "";
...
//        }
    ...
    for (var m = 0; m < titleComplements.size(); m++) {
        var titleNode = titleComplements.get(m) ;
        if ( titleNode != null && trim(titleNode.getValue()) != "") titlesCompNodes.push(titleNode) ;
    }
    
    var urlComplements = storyNode.query("//pbox [@class='complement']/p [@class='url']");
    var urlCompNodes = [] ;
    for (var m = 0; m < urlComplements.size(); m++) {
        var urlNode = urlComplements.get(m) ;
        if ( urlNode != null && trim(urlNode.getValue()) != "") urlCompNodes.push(urlNode) ;
    }

    for (var m = 0; m < Math.min(titlesCompNodes.length,urlCompNodes.length) ; m++) {
        var complementElement = new Element("complement");
        complementElement.addAttribute(new Attribute("title", titlesCompNodes[m].getValue()));
        var terms = urlCompNodes[m].getValue().split("#");
...
function processImage(imageItem) {
    ...
    var imageFile = new File(ZIP_DIR, FilenameUtils.getBaseName(ZIP_DIR) + "/" + imageItem.path);
    if (imageFile.exists()) {
...
function writeImage(imageItem, storyNode) {
    ...
    contentElement.appendChild(createElement("caption", trim(getValue(storyNode, "content/photo-groupe/photo-legende[" + imageItem.index + "]"))));
    // contentElement.appendChild(createElement("credit", trim(getValue(storyNode, "content/photo-groupe/photo-legende[" + imageItem.index + "]//credit"))));                
...
function writeImageGrp(imageItem, storyNode) {
    ...
    var photoNodes = storyNode.query("content/photo-groupe");
...
function processGraphic(graphicItem) {
    ...
    var graphicFile = new File(ZIP_DIR, FilenameUtils.getBaseName(ZIP_DIR) + "/" + graphicItem.path);
...
function writeGraphic(graphicItem, storyNode, graphicType) {
    ...
    contentElement.appendChild(createElement("caption", trim(getValue(storyNode, "content/photo-groupe/photo-legende[" + graphicItem.index + "]"))));
    contentElement.appendChild(createElement("credit", trim(getValue(storyNode, "content/photo-groupe/photo-legende[" + graphicItem.index + "]//credit"))));
...
function processGallery(galleryFile, itemNode, linkedItems) {
    ...
    var photoNodes = itemNode.query("content/galerie/photo-groupe");
...
function writeGallery(galleryNode, linkedItems) {
    _print("writeGallery : titre = " + getValue(galleryNode, "content/titraille/titre"));
    var loid = galleryNode.getAttributeValue("loid");
    var uuid = galleryNode.getAttributeValue("uuid");
    var charsCountElt = createElement("charsCount", getValue(galleryNode, "systemMetadata/props/charsCount"));
    var wordCountElt = createElement("wordCount", getValue(galleryNode, "systemMetadata/props/wordCount"));
    ...
    metaElement.appendChild(createElement("source", "LEPARISIEN/METHODE"));
    metaElement.appendChild(createElement("issueDate", getValue(galleryNode, "metadata/PubData/Paper/date_Publication")));
    metaElement.appendChild(createElement("permission", 0));

    metaElement.appendChild(createElement("extRef", uuid));
    metaElement.appendChild(charsCountElt);
    metaElement.appendChild(wordCountElt);
    var city = getValue(galleryNode, "metadata/General/GeographicalPlaces/City_Name");
    if (city != null) {
        if (city.indexOf("/") > 0) {
            var tmp = city.split("/");
            metaElement.appendChild(createElement("city", tmp[0].trim()));
    ...
    metaElement.appendChild(createElement("country", getValue(galleryNode, "metadata/General/GeographicalPlaces/Country")));
    metaElement.appendChild(createElement("address", getValue(galleryNode, "metadata/General/GeographicalPlaces/Address")));

    var catElement = new Element("categories");
    catElement.appendChild(createElement("category", getValue(galleryNode, "metadata/PubData/Paper/Section") + ""));
    metaElement.appendChild(catElement);

    var keyElement = new Element("keywords");
    keyElement.appendChild(createElement("keyword", getValue(galleryNode, "metadata/General/DocKeyword") + ""));
    metaElement.appendChild(keyElement);

    var authorsElement = new Element("authors");
    var txt = trim(getValue(galleryNode, "metadata/General/Custom_by-line"));
    txt = txt.replace(/propos recueillis par/i, "");
    txt = WordUtils.capitalize(txt) + "";
    txt = txt.replace("/ et /i", " et ");
    txt = txt.replace("/ avec /i", " avec ");
    ...
    webElement.appendChild(createElement("status", getValue(galleryNode, "metadata/PubData/Web/Status") + ""));
    webElement.appendChild(createElement("comment", getValue(galleryNode, "metadata/PubData/Web/Comment") + ""));
    webElement.appendChild(createElement("profile", getValue(galleryNode, "metadata/PubData/Web/Profile") + ""));
//    webElement.appendChild(createElement("priority", getValue(galleryNode, "metadata/PubData/Digital/Priority") + ""));
//    if ((city == null) || (city == "")) {
//        webElement.appendChild(createElement("primeRubric", ""));
//    }
	webElement.appendChild(createElement("primeRubric", getValue(galleryNode, "metadata/PubData/Digital/Priority") + ""));
    
    var topicsList = getValue(galleryNode, "metadata/PubData/Web/TopicsList") + "";
    var topics = topicsList.split('\*');
    var topicsElement = new Element("topics");
...
function processImageGal(imageGalItem) {
    ...
    var imageFile = new File(ZIP_DIR, FilenameUtils.getBaseName(ZIP_DIR) + "/" + imageGalItem.path);
    if (imageFile.exists()) {
        var imageInfo = ScriptUtils.getImageInfo(imageFile);
        if (imageInfo == null || imageInfo.getWidth() < 1 || imageInfo.getHeight() < 1) {
...
function writeImageGal(imageItem, galleryNode) {
    ...
    var photoNodes = galleryNode.query("content/galerie/photo-groupe");
    for (var i = 0; i < photoNodes.size(); i++) {
        var photoNode = photoNodes.get(i);
        var fgPhotoNodes = photoNode.query("fg-photo");
        if (fgPhotoNodes.size() > 0) {
            ...
            if (tag == "surtitre") {
                ...
                if (classAttr != null) {
                    ...
                    if (className == "tweet") {
					   ...
					   node.replaceChild(node.query("p/photo").get(0).getParent(), imgNode);
                        }
                        if (getValue(node, "p/i")) {
							var anotherStrongNode = new Element("strong");
							anotherStrongNode.appendChild(getValue(node, "p/i") + " ");
							node.replaceChild(node.query("p/i").get(0).getParent(), anotherStrongNode);
						}
                        ...
                        var strongNode = new Element("strong");
                        strongNode.appendChild(getValue(node, "p [@class='intro']") + " ");
                        var iNode = new Element("i");
                        iNode.appendChild(getValue(node, "p/i") + " ");
                        var imgNode = new Element("img");
                        var photoNodes = node.query("p/photo");
                        if (photoNodes.size() > 0) {
                            var fileref = photoNodes.get(0).getAttributeValue("fileref");
                        ...
                        node.getParent().replaceChild(node, imgNode);
                        imgNode.getParent().insertChild(strongNode, imgNode.getParent().indexOf(imgNode));
                        imgNode.appendChild(new Text(" "));
                        imgNode.getParent().insertChild(iNode, imgNode.getParent().indexOf(imgNode) + 1);
                        var pNode2 = new Element("p");
                        pNode2.appendChild(" ");
                        imgNode.getParent().insertChild(pNode2, imgNode.getParent().indexOf(imgNode) + 2);
                    }
...
function createHeadingTag(itemNode) {
    processTags(itemNode.query("content/titraille/surtitre/descendant-or-self::*"));
    return addToElement(itemNode.query("content/titraille/heading/*"), new Element("heading"));
}

function createTitleTag(itemNode) {
    processTags(itemNode.query("content/titraille/titre/descendant-or-self::*"));
    return addToElement(itemNode.query("content/titraille/title/*"), new Element("title"));
}

function createSubTitleTag(itemNode) {
    processTags(itemNode.query("content/titraille/soustitre/descendant-or-self::*"));
    return addToElement(itemNode.query("content/titraille/subtitle/*"), new Element("subtitle"));
}

function createLeadTag(itemNode) {
    processTags(itemNode.query("content/titraille/chapo/descendant-or-self::*"));
    return addToElement(itemNode.query("content/titraille/lead/*"), new Element("lead"));
}

function createLegPhotoTag(itemNode) {
    processTags(itemNode.query("content/photo-groupe/photo-legende/descendant-or-self::*"));
    return addToElement(itemNode.query("content/photo-groupe/legend/*"), new Element("legend"));
}

function createCreditPhotoTag(itemNode) {
    processTags(itemNode.query("content/photo-groupe/photo-legende/p/credit"));
    return addToElement(itemNode.query("content/photo-groupe/photo-legende/p/creditlegend"), new Element("creditlegend"));
}

function createPreLegPhotoTag(itemNode) {
    processTags(itemNode.query("content/photo-groupe/photo-legende/p/b"));
    return addToElement(itemNode.query("content/photo-groupe/photo-legende/p/b"), new Element("prelegend"));
}

function createTexteTag(itemNode) {

    var nodes = itemNode.query("content/texte/descendant-or-self::* | content/tables/descendant-or-self::*");
    processTags(nodes);

    var textNode = new Element("text");
    nodes = itemNode.query("content/text/*");
    for (var i = 0; i < nodes.size(); i++) {
