...
function createXML() {
    ...
    for (var i in courseArray) {
        ...
        thead0.addAttribute(new Attribute("class", "header_PAR" )) ;
        ...
        thead1.addAttribute(new Attribute("class", course.classe + "_header_starters0" )) ;
		...
		{
			td30.appendChild("Tiercé - Quarté+ - Quinté+ - 2sur4 - Trio - Couplés - Trio O. - Couplé O.") ;
		}
        ...
        if ( course.type.matches(".*Tierc.*Quart.*Quint.*") ) {
            ...
            // Display a table of header first
            var thead2 = new Element("thead");
            thead2.addAttribute(new Attribute("class", course.classe + "_header_starters1" )) ;
