...
function createXmlPages() {
    ...
    // product-LP
    var dirA = new File(ZIP_DIR, DATE_NAME + "/product-LP/").listFiles();
    for (var i = 0; i < dirA.length; i++) {
        // product-LP/edition-PAR75-1.0.14218756
        var p1 = DATE_NAME + "/product-LP/" + dirA[i].getName();
        var dirB = new File(dirA[i]).listFiles();
        for (var j = 0; j < dirB.length; j++) {
            // product-LP/edition-PAR75-1.0.14218756/book-b
   		  var p2 =  p1 + "/" + dirB[j].getName();
            var dirC = new File(dirB[j]).listFiles();
   		  for (var k = 0; k < dirC.length; k++) {
        		 // product-LP/edition-PAR75-1.0.14218756/book-b/page-41-1.0.14553253
	   		 var p3 = p2 + "/" + dirC[k].getName();
	           var dirD = new File(dirC[k]).listFiles();
	           for (var l = 0; l < dirD.length; l++) {
	        		var path = p3 + "/" + dirD[l].getName();
	        		var filename = new File(dirD[l]).getPath();
                    if (FilenameUtils.isExtension(filename, "xml") && (filename.indexOf("index")===-1)) {
                    	_print("filename Page: " + filename);
    ...
    for (var i = 0; i < pxpStoryNodes.size(); i++) {
        ...
        for (var j = 0; j < pxpContentNodes.size(); j++) {
            ...
            if (!sameLoid && linkType != "other") {
                var itemNodes = pageDoc.query("/page/printItems/item[@loid='" + loid + "']");
                if (itemNodes.size() > 0) {
...
function writePage(physPage) {
    ...
    for (var h = 0; h < phys.length; h++) {
        ...
        var pdfSrcName = FilenameUtils.getBaseName(phy.path) + "-PDF.pdf";
        var pdfSrcFile = new File(xmlSrcFile.getParent(), pdfSrcName);
...
function createAffine(contentNodes) {
    ...
    if (contentNodes.size() > 0) {
        var str = contentNodes.get(0).getAttributeValue("contentTm2") + "";
...
function createShape(shapeNodes, shape) {
    ...
    for (var i = 0; i < shapeNodes.size(); i++) {
        ...
        var x = shapeNode.getAttributeValue("shapeNodeX");
        if (x != null)
            x = parseFloat(x);

        var y = shapeNode.getAttributeValue("shapeNodeY");
        if (y != null)
            y = parseFloat(y);

        var xx = shapeNode.getAttributeValue("shapeNodeWidth");
        if (xx != null)
            xx = x + parseFloat(xx);

        var yy = shapeNode.getAttributeValue("shapeNodeHeight");
        if (yy != null)
            yy = y + parseFloat(yy);
...
function processStory(physPage, container, storyItem, pageDoc) {
    ...
    // Item node
    var itemNode = pageDoc.query("/page/printItems/item[@loid='" + storyItem.loid + "']").get(0);
    ...
    // Ours
    var nodes = itemNode.query("content/texte");
    ...
    var nodes = itemNode.query("content/titraille");
    for (var i = 0; i < nodes.size(); i++) {
        var txt = nodes.get(i).getValue();
        mycharsCount += txt.length();
    }

    // var charsCount = getValue(itemNode, "systemMetadata/props/charsCount");
    if (storyItem.items.length == 0 && mycharsCount < 75) {
        _print("Story: " + storyItem.loid + " filtered - too small text! - Page: " + physPage.sequence + " -  charsCount: " + mycharsCount);
        storyItem.isValid = false;
    }
...
//    return addToElement(itemNode.query("content/titraille/heading/*"), new Element("heading"))
    ...
    var nodes = itemNode.query("content/titraille/surtitre/descendant-or-self::*");
    processTags(nodes);

    var headingNode = new Element("heading");

    nodes = itemNode.query("content/titraille/heading/*");
    for (var i = 0; i < nodes.size(); i++) {
...
function createTexteTag(itemNode) {
    ...
    if (size > 4) {
        ...
        if (maxInd < size) {
            nodes = itemNode.query("content/text[" + (maxInd + 1) + "]/*");
            for (var i = 0; i < nodes.size(); i++) {
                var node = nodes.get(i);
                if (node.getChildCount() > 0) {
                    node.detach();
...
function writeStory(physPage, container, storyItem, pageDoc) {
    ...
    //var itemNode = pageDoc.query("/page/printItems/item[@loid='" + storyItem.loid + "']").get(0);
    var itemNodes = pageDoc.query("/page/printItems/item[@loid='" + storyItem.loid + "']");
    ...
    var charsCountElt = createElement("charsCount", getValue(itemNode, "systemMetadata/props/charsCount"));
    var wordCountElt = createElement("wordCount", getValue(itemNode, "systemMetadata/props/wordCount"));
    ...
    var city = getValue(itemNode, "metadata/General/GeographicalPlaces/City_Name");
    if (city != null) {
        var tmp = city.split("/");
    ...
    metaElement.appendChild(createElement("province", getValue(itemNode, "metadata/General/GeographicalPlaces/Province_Name")));
    metaElement.appendChild(createElement("country", getValue(itemNode, "metadata/General/GeographicalPlaces/Country")));
    var address = getValue(itemNode, "metadata/General/GeographicalPlaces/Address");
    ...
    catElement.appendChild(createElement("category", (getValue(itemNode, "metadata/PubData/Paper/Section") + "").replace("Rituel", physPage.section)));
    ...
    keyElement.appendChild(createElement("keyword", getValue(itemNode, "metadata/General/DocKeyword") + ""));
    metaElement.appendChild(keyElement);

    var txt = trim(getValue(itemNode, "metadata/General/DocAuthor")) + "";
    ...
    webElement.appendChild(createElement("theme", getValue(itemNode, "metadata/PubData/Web/Theme") + ""));
    webElement.appendChild(createElement("status", getValue(itemNode, "metadata/PubData/Web/Status") + ""));
    webElement.appendChild(createElement("comment", getValue(itemNode, "metadata/PubData/Web/Comment") + ""));
    webElement.appendChild(createElement("profile", getValue(itemNode, "metadata/PubData/Web/Profile") + ""));
    webElement.appendChild(createElement("priority", getValue(itemNode, "metadata/PubData/Digital/Priority") + ""));

    var topicsList = getValue(itemNode, "metadata/PubData/Web/TopicsList") + "";
    ...
    var authorList = [];
    var txt = trim(getValue(itemNode, "metadata/General/Custom_by-line")) + "";
...
function processGallery(physPage, storyItem, galleryItem, pageDoc) {
    ...
    var galleryNode = pageDoc.query("/page/printItems/item[@loid='" + galleryItem.loid + "']").get(0);
...
function writeGallery(physPage, storyItem, galleryItem, pageDoc) {
    ...
    var itemNode = pageDoc.query("/page/printItems/item[@loid='" + galleryItem.loid + "']").get(0);
    ...
    metaElement.appendChild(charsCountElt);
    metaElement.appendChild(wordCountElt);
    metaElement.appendChild(createElement("city", getValue(itemNode, "metadata/General/GeographicalPlaces/City_Name")));
    metaElement.appendChild(createElement("country", getValue(itemNode, "metadata/General/GeographicalPlaces/Country")));
    metaElement.appendChild(createElement("address", getValue(itemNode, "metadata/General/GeographicalPlaces/Address")));

    var catElement = new Element("categories");
    catElement.appendChild(createElement("category", getValue(itemNode, "metadata/PubData/Paper/Section") + ""));
    metaElement.appendChild(catElement);
...
function processImage(physPage, container, imageItem, pageDoc) {
    ...
    if (imageFile.exists()) {
        ...
        //var itemNode = pageDoc.query("/page/printItems/item[@loid='" + imageItem.loid + "']").get(0);
    ...
    for (var k = 0; k < container.length; k++) {
        ...
        if (itm2.linkType == "story") {
            ...
            if (item.isValid) {
                ...
                var storyNodes = pageDoc.query("/page/printItems/item[@loid='" + item.loid + "']");
...
function processGraphic(physPage, container, graphicItem, pageDoc) {
    ...
    if (graphicFile.exists()) {
        ...
        //var itemNode = pageDoc.query("/page/printItems/item[@loid='" + graphicItem.loid + "']").get(0);
    ...
    for (var k = 0; k < container.length; k++) {
        ...
        if (itm2.linkType == "story") {
            ...
            if (item.isValid) {
                ...
                var storyNode = pageDoc.query("/page/printItems/item[@loid='" + item.loid + "']").get(0);
